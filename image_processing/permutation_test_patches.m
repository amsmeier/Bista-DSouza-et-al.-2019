...
if exist('baselineFile','var') && ~isempty(baselineFile) % if baseline file provided
    if ischar(baselineFile)
        baselineimg = imread(baselineFile);
    else
        baselineimg = baselineFile; 
    end
    baselineimg = baselineimg(:,:,1); % in case image is rgb, take only first channel
    if numel(unique(baselineimg(baselineimg~=0))) ~= 1
        error('More or less than 1 unique non-zero pixel value found in baseline file.')
    end
    baselineimg = logical(baselineimg); 
