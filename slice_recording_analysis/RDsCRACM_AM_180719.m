...
% Last modified by AM 1/7/19 on recording comp. 
...
normto_celltotal = 1;       %% normalize all pixel current values to fraction of total current input into this neuron 
...
celltypes = {'patch';'interpatch'};   %% names of cell types; should correspond to columns in CellList
pixelheight = 75;           %% height of each pixel in um for plotting
L1_pixelrows = [1:2];       %% rows of pixels from the top of the image representing Layer 1 (vector)
L2_4_pixelrows = [3:6];     %% rows of pixels from the top of the image representing Layers 2-4 (vector)
L4_pixelrows = [7:8];
...
% Plotting options
plot_group_images={1,'uscaled',1}  ; %1st arg: plot group mean; 2nd arg: set to 'scaled' to scale colors within each plot; 3rd arg: resolution enhancement factor (=1 for original image
...
plot_vertdist = {1, 'overlay', 'nogrid'};  % argument 1 = 1 for plotting on; argument 2 = 'overlay' to plot all cases on the same graph; argument 3 = 'grid' to display grid lines
    vertdist_all_on_one_plot = 1; % plot all cell types together.... may cause error when also plotting multiple cases
    vdist_colors = {[0 0 1], [1 0 0], [0 1 0]}; 
plot_vertdist_difference = 0;       % plot difference between input into each row in first pathway vs. second pathway
plot_layersum = 1;           % plot groups of total inputs to L1 and to L2-4
...
for m=1:size(CaseList,1)     %% for each case/pathway
    ...
    for c = 1:size(CellList,2)   %% for each type of cell (PV and pyramidal)
        ...
        for r = 2:size(CellList,1)    %% for each cell listed (including repeats)
            %%% if entry isn't blank, and as long as no_repeats is off or the name isn't a repeat of a preceiding row...
            this_cell_name = CellList{r,c};
            if  ~isempty(this_cell_name) && ~(no_repeats && any(strcmp(this_cell_name,CellList(1:r-1,c))));   
    ...
    for thiscase = 1:size(CaseList,1)   % one column for each case
        for thistype = 1:size(celltypes,1)    %% one subplot for each cell type
            if strcmp(plot_vertdist{2},'overlay')
                ...
                if vertdist_all_on_one_plot
                    hold on
                    thiscolor = vdist_colors{thistype};
                    title(casename)
                elseif ~vertdist_all_on_one_plot
                    ...
                    thiscolor = vdist_colors{1}; 
                end
                if thiscase == size(CaseList,1);    %% if this is the final case
                    h = [h; herrorbar(vdist_forplotting(:,:,thistype),repmat((fliplr(1:size(vdist_forplotting,1)))',1,size(vdist_forplotting,2)),...
                        verror_forplotting(:,:,thistype),verror_forplotting(:,:,thistype))];
                    set(h(end-1:end),'Color',thiscolor);
                    l = legend(h(1:2:end),celltypes);
...
if plot_layersum
   ...
   barweb([(extractfield(all_cases,'L1_PV_groupmean'))',(extractfield(all_cases,'L2_4_PV_groupmean'))'],...
       ...
       [],CaseList','Patch Cells Input Received in L1 and L2-4',[],'Normalized Input',[],[],{'L1','L2-4'}); %% plot PV L1 and L2-4 sums
   set(gca,'Fontsize',9)
   ...
   barweb([(extractfield(all_cases,'L1_pyr_groupmean'))',(extractfield(all_cases,'L2_4_pyr_groupmean'))'],...
       ...
       [],CaseList','Interpatch Cells Input Received in L1 and L2-4',[],'Normalized Input',[],[],{'L1','L2-4'}); %% plot PV L1 and L2-4 sums
   set(gca,'Fontsize',9); set(gcf,'Position',[1300 300 620 500])    %% font size, position, window size 
end
